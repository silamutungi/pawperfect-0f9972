    ...
    <div style={{ background: "var(--color-bg)", minHeight: "100vh" }}>
      ...
      <main className="flex items-center justify-center px-6 py-24">
        <div className="w-full max-w-sm">
          <h1 className="font-bold mb-2" style={{ fontSize: "var(--text-title-1)" }}>Create your account</h1>
          <p className="mb-8" style={{ color: "var(--color-text-secondary)", fontSize: "var(--text-subhead)" }}>Join the marketplace — find groomers or list your services, free.</p>
          {error && (
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
            <div className="flex flex-col gap-1.5">
              ...
              <Input id="name" type="text" autoComplete="name" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Smith" />
            </div>
            <div className="flex flex-col gap-1.5">
            ...
            <Button type="submit" disabled={loading} className="w-full" style={{ background: "var(--color-primary)", color: "#fff", minHeight: "44px" }}>
              {loading ? "Creating account..." : (<><UserPlus size={16} aria-hidden="true" /> Create Account</>)}
            </Button>
          </form>
